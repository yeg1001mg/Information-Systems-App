...
export function* updateUserProfile(action: PayloadAction<UpdateUserProfilePayload>) {
    try {
...
export function* getCurrentUserProfileData(action: PayloadAction<GetUserProfileDataPayload>) {
    try {
        const { uid } = action.payload;
        //@ts-ignore
        const userProfileData = yield call(getUserProfile, uid)
        if (userProfileData.exists()) {
            yield put(AuthActions.setCurrentUserAdditionalData(userProfileData.data()))
        } else {
...
export default function* groupsSagas(): SagaGenerator<StrictEffect> {
    yield all([
        ...
        yield takeLatest(AuthActions.getCurrentUserProfileData, getCurrentUserProfileData),
